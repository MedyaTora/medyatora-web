...
function normalizeCustomer(
  customers: AnalysisRequestRow["customers"]
): CustomerInfo | null {
...
export default async function AdminPage() {
  ...
  if (orderError) {
    return <ErrorScreen message={orderError.message} />;
  }

  if (customerCountError) {
    return <ErrorScreen message={customerCountError.message} />;
  }
  ...
  const newRequests = allItems.filter(
    (item) => item.package_type === "analysis" && item.status === "pending"
  );
  ...
  const completedRequests = allItems.filter(
    (item) => item.package_type === "analysis" && item.status === "completed"
  );

  const otherPackages = allItems.filter(
    (item) => item.package_type !== "analysis"
  );

  const pendingOrders = allOrders.filter((item) => item.status === "pending");
  const processedOrders = allOrders.filter((item) => item.status !== "pending");
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,#171717_0%,#090909_55%,#050505_100%)] p-4 text-white md:p-8">
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <AnalysisSection title="Yeni Başvurular" items={newRequests} />
        <AnalysisSection title="İşlemdekiler" items={processingRequests} />
        <AnalysisSection title="Tamamlananlar" items={completedRequests} />
        <AnalysisSection title="Diğer Paketler" items={otherPackages} />
            ...
            return (
              ...
              >
                ...
                <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  <InfoBox label="Kullanıcı Adı" value={customer?.username || "-"} />
                  <InfoBox
                    label="İletişim"
                    value={`${customer?.contact_type || "-"} / ${customer?.contact_value || "-"}`}
                  />
                  <InfoBox label="Hesap Linki" value={customer?.account_link || "-"} />
                  <InfoBox label="Hesap Türü" value={customer?.account_type || "-"} />
                  <InfoBox label="İçerik Türü" value={customer?.content_type || "-"} />
                  <InfoBox
                  ...
                  />
                  <InfoBox label="Ana Problem" value={customer?.main_problem || "-"} />
                  <InfoBox label="Ana Eksik" value={customer?.main_missing || "-"} />
                </div>
              </div>
            );
          })}
            ...
            >
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                ...
                <InfoBox
                  ...
                  value={formatMoney(item.unit_cost_price, item.currency)}
                />
                <InfoBox
                  label="1000 Adet Satış"
                  value={formatMoney(item.unit_price, item.currency)}
                />
                <InfoBox
                  label="Toplam Alış"
                  value={formatMoney(item.total_cost_price, item.currency)}
                />
                <InfoBox
                  label="Toplam Satış"
